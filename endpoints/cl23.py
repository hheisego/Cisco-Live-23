...
class CiscoLive:
    ...
    def get_co2(self):
        ...
        response = requests.request(
            "GET", url + resource, headers=headers, data=payload)
    ...
    def meraki_data(self, meraki_post):
        ...
        co2_levels = "\nTimestamp: " + \
            str(datetime.today().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3])
        ...
        if meraki_post['type'] == 'WiFi':
            ...
            payload = {"timestamp": timestamp(),
                       "ssid": ssid if ssid != '' else "nearby user",
                       "ap_connected": ap_connected,
                       "nearby": nearby,
                       "CO2": co2,
                       "temperature": random.randrange(20, 28),
                       "persons_count": room_count
                       }
            
            livecrud.create(payload, table='Reports')
...
# FastAPI Object
app = FastAPI(title='Cisco Live! 2023',
              description='new release', version='1.1.3')
...
async def info_insert(request: Request):
    ...
    livecrud.create(raw_info, table='raw_info')
...
async def get_users(request: Request):
    ...
    return cisco_live.get(table='Reports')


if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=5007,
                ssl_keyfile="private.key", ssl_certfile="te-health_nginx.crt")
